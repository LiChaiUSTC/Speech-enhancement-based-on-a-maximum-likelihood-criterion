...
#define MAX_VECTOR_NUM 			10000000
#define MAX_SAMPLE_NUM			100000000


#define OLA_KIND				1
#define POSTPROCESS				0
#define SMOOTHPROCESS			0
#define SMOOTH_WIN				1
...
int SamplingFrequency = 16000,  /* SamplingFrequency */
  NativeByteOrder = IEEE_BE,    /* Native byte ordering */
...
{
	float** out_proc_vectors = (float**)malloc((MAXWINDOWSIZE + 1) * sizeof(float*));
	for(int i=0; i< MAXWINDOWSIZE + 1; i++)
	{
		out_proc_vectors[i] = (float*)malloc(MAX_VECTOR_NUM * sizeof(float));
	}
	float** clean_spec_vectors = (float**)malloc((MAXWINDOWSIZE + 1) * sizeof(float*));
	for(int i=0; i< MAXWINDOWSIZE + 1; i++)
	{
		clean_spec_vectors[i] = (float*)malloc(MAX_VECTOR_NUM * sizeof(float));
	}
	float** noisy_spec_vectors = (float**)malloc((MAXWINDOWSIZE + 1) * sizeof(float*));
	for(int i=0; i< MAXWINDOWSIZE + 1; i++)
	{
		noisy_spec_vectors[i] = (float*)malloc(MAX_VECTOR_NUM * sizeof(float));
	}
	float** denoise_spec_vectors = (float**)malloc((MAXWINDOWSIZE + 1) * sizeof(float*));
	for(int i=0; i< MAXWINDOWSIZE + 1; i++)
	{
		denoise_spec_vectors[i] = (float*)malloc(MAX_VECTOR_NUM * sizeof(float));
	}
    ...
    if (ParseCommLine (argc - 1, argv + 1))
    {
		...
		for(j=0; j<InHheader.nSamples; j++)
		{
			...
			Swap_float_ptr((void*)temp_vectors, (FFTLength/2+1));
