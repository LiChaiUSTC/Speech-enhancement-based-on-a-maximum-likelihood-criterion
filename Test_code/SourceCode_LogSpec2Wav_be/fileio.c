...
ReadHTKHeader (FILE * fp_in, HTK_Header * header, int Swap )
{
    ...
    fseek (fp_in, 0L, SEEK_SET);
    if (!fread (&(header->nSamples), sizeof (int), 1, fp_in))
          return 0;
    if (!fread (&(header->sampPeriod), sizeof (int), 1, fp_in))
          return 0;
    ...
    if (header->nSamples < 0 || header->sampPeriod < 0 ||!ParseHTKParmKind (header->sampKind, str))
        {
...
void Swap_float_ptr(void *fea, int size)
{
	char *ptr;
	char temp;
	int i;
