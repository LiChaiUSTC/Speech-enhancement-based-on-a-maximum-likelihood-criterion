...
SOUND *sndOpen PARG((SOUND *snd, char *path, char *mode));
...
void sndPrint PARG((SOUND *snd, FILE *stream, char *tag));
    /* print a text description of a snd struct */
void sndDebugPrint PARG((SOUND *snd, FILE *stream, char *tag));
...
int sndSetSFformatByName PARG((SOUND *snd, char *name));
