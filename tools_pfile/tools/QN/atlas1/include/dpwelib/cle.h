...
typedef int (*cleSpecialSetFn)(char *arg, void *where, char *flag, void *data);
...
typedef struct {    /* a command line entry */
    char *flag;     /* e.g. "-n", or "" for any-match (must be at bottom) */
...
void cleUsage(CLE_ENTRY *entries, char *programName);
